...
- **★★ Realised gains card, closed positions panel, and the Return Table cleanup
  ...
  **Things a future session needs to know before touching this:**
  - **★ MONO IS RETIRED FROM THE CLIENT DASHBOARD ENTIRELY (2026-09-10) — and, as of
    ...
    as an accent. Step two took the figures too, because with the labels already Inter, a
    table of monospaced numbers under Inter headings still read as two typefaces bolted
    together. The `.ret-mono` utility is renamed `.ret-fig` — a class called "mono" that no
    longer selects a monospace family is worse than no class at all.
  - **★ THE ALIGNMENT MONO PROVIDED IS KEPT, AND LOSING IT IS THE REAL RISK IF ANYONE EDITS
    THIS.** Equal digit widths were the only genuinely functional reason to use mono for money
    columns, and Inter's default figures are PROPORTIONAL: measured at 13px, "1111111" renders
    37.78px against 58.75px for "0000000", so one `1` in a column visibly staggers it.
    `font-variant-numeric: tabular-nums` fixes it — measured, both strings render 58.97px,
    and $50,000 and $59,200 both 54.02px. It is declared on `.rt` so the whole table inherits
    it, and on the card figure classes. **Do not drop it while keeping the family change**:
    that trades a font mismatch for a misaligned money column, which is worse and less
  ...
  **The rest of the app was audited and then brought along.** dashboard.html had used mono in
  exactly four places — `.ret-pc`, `.ret-u`, `.ret-r` and the old `.ret-mono` — every one a
  figure, with its card labels already Inter 600; all four are Inter with tabular figures now,
  so the two linked pages cannot disagree about how a percentage is set. The one remaining
  `font-mono` in the client app is `support.html`'s ticket id, one per row — an identifier,
  genuinely sparse, deliberately left. Nothing else in the client or admin family uses mono.
