...
//
// Real Edge Function port of engine-core.js's rejectWithdrawal(clientId, requestId, reason) —
// read that function's real source in full before writing this, not reinvented. Marks a
// pending withdrawal request rejected with a reason; moves nothing (no account_state or
  ...
  try {
    ...
    if (clientRow) {
      const { html, text } = renderEmail({
        heading: 'An update on your Marketswave withdrawal request',
        introParagraphs: ['Hi ' + clientRow.name + ', your withdrawal request could not be approved. Please contact support if you have any questions.'],
