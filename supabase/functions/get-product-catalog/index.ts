// ★ PM tool revamp, part 6 (2026-09-16) — the product catalogue, in one read.
//
...
//
// A pure read with no side effect: it settles the catalogue (which every client read path
// already does on every call) and touches no counter a PM's own visit could move.
import { createClient } from 'jsr:@supabase/supabase-js@2';
import { corsHeaders } from '../_shared/cors.ts';
import { buildProductCatalog } from '../_shared/product-catalog.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const catalog = await buildProductCatalog(admin);
