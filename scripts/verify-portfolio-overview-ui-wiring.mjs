...
async function main() {
  ...
  try {
    ...
    // Chart section.
    check('the chart section is titled "Portfolio value over time", states its own scope (savings sit outside it), carries the legend, and states no dollar figure', card.querySelector('.po-ch-title').textContent === 'Portfolio value over time' && /Charts the portfolio only/.test(card.querySelector('.po-ch-scope').textContent) && /Savings pockets sit outside/.test(card.querySelector('.po-ch-scope').textContent) && !D.getElementById('po-legend').hidden && /Portfolio value/.test(D.getElementById('po-legend').textContent) && /Capital you put in/.test(D.getElementById('po-legend').textContent) && !/\$/.test(card.querySelector('.po-ch-h').textContent), card.querySelector('.po-ch-h').textContent);
