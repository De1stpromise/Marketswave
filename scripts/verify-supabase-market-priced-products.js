...
async function main() {
  ...
  try {
    ...
    check('★ rotation accounting: N per run = 30, worst-case staleness = ceil(stocks / 30) x 5 min (the 5-minute cadence, row 211), headroom to the next cycle', refresh.body.stockSymbolsPerRun === 30 && refresh.body.worstCaseStalenessMinutes === Math.ceil(refresh.body.distinctStockSymbols / 30) * refresh.body.refreshIntervalMinutes && refresh.body.refreshIntervalMinutes === 5 && refresh.body.headroom === Math.ceil(refresh.body.distinctStockSymbols / 30) * 30 - refresh.body.distinctStockSymbols, JSON.stringify(refresh.body));
